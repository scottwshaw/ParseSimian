Feature: ParseSimianFile
  ...
  As a programmer
  I want to parse the
 results of a Simian file and produce JSON that can be consumed by protoviz
  
  Scenario: Parse file and dump
    Given I have a Simian output file "simian_report.xml"
    When I run the parser on that file
    Then it should produce json output starting with "var\s*simian_report\s*=\s*\{nodes:\[\{nodeName"
    And end with "\s*}\s*;\s*"
